...
# TODO: longitudinal Pacejka tire forces are missing   
# TODO: consider down force!

def sim_stm(veh_params_file, tire_params_file = "pacejka_params_file.yaml"):   
    """
    ...
    model_name  = "simple_dynamic_bicycle_model"
    # load vehicle params
    ...
    g = 9.81  #[m/s^2]
    banking = deg2rad(veh_params['banking_deg'] )
    ...
    x = vertcat(posx, posy, yaw, vlong, vlat, yawrate, delta_f)

    # controls
    a                   = MX.sym("a")               # acceleration
    steering_rate       = MX.sym("steering_rate")
    u                   = vertcat(a, steering_rate)
    ...
    # Define the disturbance range for each state derivative
    # disturbance_range = {'posx_dot': [-0.1, 0.1], 'posy_dot': [-0.1, 0.1], 'yaw_dot': [-0.01, 0.01], 
    #                  'vlong_dot': [-1.0, 1.0], 'vlat_dot': [-1.0, 1.0], 'yawrate_dot': [-0.1, 0.1], 
    #                  'delta_f_dot': [-0.1, 0.1]}
    # Define the disturbance variable and its bounds
    ...
    # disturbance_bounds = [disturbance_range[key] for key in disturbance_range.keys()]
    w = vertcat(w_posx, w_posy, w_yaw, w_vlong, w_vlat, w_yawrate, w_delta_f)
    
    # make control inputs + disturbance input as parameters (CasADi integrators accept only states and paramters)
    u_disturbed = vertcat(a, steering_rate, w_posx, w_posy, w_yaw, w_vlong, w_vlat, w_yawrate, w_delta_f)
    # parameters
    # p = vertcat(a, steering_rate)
    ...
    xdot = vertcat(posx_dot, posy_dot, yaw_dot, vlong_dot, vlat_dot, yawrate_dot, delta_f_dot)
    xdot_disturbed = xdot + w
                    # vertcat(posx_dot + w_posx,
                    #         posy_dot + w_posy, 
                    #         yaw_dot + w_yaw, 
                    #         vlong_dot + w_vlong, 
                    #         vlat_dot + w_vlat, 
                    #         yawrate_dot + w_yawrate, 
                    #         delta_f_dot + w_delta_f)
    ...
    ## Initial Conditions
    x0 = np.array([0, 0, 0, 0, 0.00, 0, 0.0])
    w0 = np.array([0, 0, 0, 0, 0.00, 0, 0.0])
    ## constraints
    # constraint on lateral acceleration
    a_lat       = MX.sym("a_lat")
    a_lat       = vlong * yawrate
    # state bounds
    ...
    # define nonlinear constraint struct
    constraint.alat = Function("a_lat", [x], [a_lat])
    constraint.expr = vertcat(a_lat)
    ## CasADi model struct
    model.f_expl_expr   = f_expl
    model.x             = x
    model.xdot          = xdot
    model.xdot_disturbed= xdot_disturbed 
    model.u             = u
    model.w             = w
    model.z             = z
    # model.p             = p
    model.u_disturbed   = u_disturbed
