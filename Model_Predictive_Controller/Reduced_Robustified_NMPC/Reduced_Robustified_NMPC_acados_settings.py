# Created on Fri Apr 21 14:00 2023
...
from Model_Predictive_Controller.Reduced_Robustified_NMPC.Robust_NMPC_pred_model_utils import pred_stm_disturbed, P_propagation
import scipy.linalg
import numpy as np
from casadi import vertcat, sqrt, fmod, pi, if_else, MX, Function, gradient

"""
Source: [1] Zarrouki, B., Nunes, J., & Betz, J. (2023). R $^ 2$ NMPC: A Real-Time Reduced Robustified Nonlinear Model Predictive Control with Ellipsoidal Uncertainty Sets for Autonomous Vehicle Motion Control. https://arxiv.org/abs/2311.06420
"""
...
def acados_settings(Tf, N, x0, Q, R, Qe, L1_pen, L2_pen, ax_max_interpolant, ay_max_interpolant, combined_acc_limits, veh_params_file = "veh_parameters_bmw320i.yaml", tire_params_file= "pacejka_params_file.yaml", solver_generate_C_code = True, solver_build = True):
    ...
    pred_model, constraints = pred_stm_disturbed(veh_params_file, tire_params_file)
    ...
    if combined_acc_limits == 0:
        ...
        gradient_vector = [Function('grad1', [x], [gradient(a_lon_limits_ineq, x)]),
                            Function('grad2', [x], [gradient(a_lat_limits_ineq, x)])]
        ...
        gradient_vector = [Function('grad1', [x], [gradient(acc_limits_ineq1, x)]),
                           Function('grad2', [x], [gradient(acc_limits_ineq2, x)])]
        ...
        gradient_vector = [Function('grad1', [x], [gradient(acc_limits_ineq, x)])]
    
    # The gradient vector is in the end exported so that the computation of the gradient can be done in the 
    # main file, computing the acceleration constraints 
    ...
    steering_grad = Function('grad_steering', [x], [gradient(x[6], x)])
    # stage bounds on u 
    ...
    # create solver
    acados_solver = AcadosOcpSolver(ocp, json_file="acados_ocp_R2NMPC.json", generate = solver_generate_C_code, build=solver_build)

    return constraints, pred_model, acados_solver, ocp.cost.cost_type, ocp, gradient_vector, steering_grad
